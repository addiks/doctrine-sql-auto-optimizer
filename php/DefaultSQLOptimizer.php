...
final class DefaultSQLOptimizer implements SQLOptimizer
{
    ...
    public function optimizeSql(string $inputSql, Schemas $schemas): string
    {
        ...
        /** @var array<SqlAstNode> $nodes */
        $nodes = $this->sqlParser->parseSql($inputSql);
         ...
         * @return Mutator
         */
        $mutators = array_map(function (Closure $mutator): Closure {
        ...
        foreach ($nodes as $node) {
            if ($node instanceof SqlAstMutableNode) {
                $node->mutate($mutators);
            }

            $outputSql .= $node->toSql();
        }
